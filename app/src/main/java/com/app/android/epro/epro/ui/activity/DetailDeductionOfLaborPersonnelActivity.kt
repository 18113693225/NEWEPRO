...
import com.app.android.epro.epro.ui.adapter.DetailDeductionOfLaborPersonnelAdapter
...
import kotlinx.android.synthetic.main.detail_deduction_of_labor_personnel.*
import kotlinx.android.synthetic.main.include_detail_bar.*
import kotlinx.android.synthetic.main.include_detail_top.*
import org.greenrobot.eventbus.EventBus

class DetailDeductionOfLaborPersonnelActivity : BaseDetailActivity(), ProcessInfoContract.View,
    InfoBottomFragment.RefreshActivity {

    private lateinit var info: DetailDeductionOfLaborPersonnelBean
    private lateinit var id: String
    private lateinit var menu: String
    private lateinit var jobId: String
    private var from: String = "-1"
    private var mAdapter: DetailDeductionOfLaborPersonnelAdapter? = null
    ...
    override fun layoutId(): Int {
        return R.layout.detail_deduction_of_labor_personnel
    }
    ...
    override fun setDetailInfoData(data: Any) {
        info = data as DetailDeductionOfLaborPersonnelBean
        when (info.code) {
            0 -> {
    ...
    private fun setView(info: DetailDeductionOfLaborPersonnelBean.Data.Object) {
        ...
        workerDeductType.text =
            if (info.workerDeductType == "1") "临时劳务" else "分包劳务"
        workerContractName.text = info.workerContractName
        workerProjectManagerName.text =
            if (info.workerProjectManagerName.isEmpty()) CustomUtils.emptyInfo else info.workerProjectManagerName
        workerDeductRemark.text =
            if (info.workerDeductRemark.isEmpty()) CustomUtils.emptyInfo else info.workerDeductRemark




        if (info.deductDetailList.isNullOrEmpty()) {
            list_ll.isVisible = false
        } else {
            list_ll.isVisible = true
            mAdapter =
                DetailDeductionOfLaborPersonnelAdapter(info.deductDetailList as MutableList<DetailDeductionOfLaborPersonnelBean.Data.Object.DeductDetail>)
