...
import com.app.android.epro.epro.mvp.model.bean.DetailTaxCertificateBean
...
import kotlinx.android.synthetic.main.detail_tax_ertificate.*
import kotlinx.android.synthetic.main.include_detail_bar.*
import kotlinx.android.synthetic.main.include_detail_top.*
import org.greenrobot.eventbus.EventBus

class DetailTaxCertificateActivity : BaseDetailActivity(), ProcessInfoContract.View,
    InfoBottomFragment.RefreshActivity {

    private lateinit var info: DetailTaxCertificateBean
    ...
    override fun layoutId(): Int {
        return R.layout.detail_tax_ertificate
    }
    ...
    override fun setDetailInfoData(data: Any) {
        info = data as DetailTaxCertificateBean
        when (info.code) {
            0 -> {
    ...
    private fun setView(info: DetailTaxCertificateBean.Data.Object) {
        ...
        taxProjectName.text = info.taxProjectName
        taxPartyName.text = info.taxPartyName
        taxApplyUserName.text = info.taxApplyUserName
        taxApplyUnitName.text = info.taxApplyUnitName
        taxApplyDepartmentName.text = info.taxApplyDepartmentName
        taxApplicantMoney.text = info.taxApplicantMoney.toString()
        taxUpperMoney.text = info.taxUpperMoney

        taxNotice.text =
            if (info.taxNotice.isEmpty()) CustomUtils.emptyInfo else info.taxNotice
